...
from sklearn.linear_model import SGDRegressor  # ML Library Requirement
...
# Pre-optimized trial configuration matching descriptive names
TRIALS = [
...
LOG_FILE = "part2_sklearn_trials_log.csv"
# ----------------------------------------
...
# 6. WRAPPER SKLEARN MODEL EXECUTION
trial_logs = []
best_mse = float("inf")
best_model = None

for idx, trial in enumerate(TRIALS):
    # Mapping custom dictionary configurations safely into native library signatures
    model = SGDRegressor(
        loss='squared_error',
        learning_rate='constant',
        eta0=trial["learning_rate"],  
        max_iter=trial["iterations"], # Handled correct argument mapping safely
        tol=1e-9,
        penalty=None,
...
# 7. GENERATE MATCHING SKLEARN FEATURE VISUALIZATION PLOT
best_feat_idx = np.argmax(np.abs(best_model.coef_))
best_feat_name = X_all.columns[best_feat_idx]

plt.figure(figsize=(7, 5))
plt.scatter(X_test.iloc[:, best_feat_idx], y_test, color='darkgreen', alpha=0.6, label="Actual Targets")
plt.scatter(X_test.iloc[:, best_feat_idx], best_model.predict(X_test_scaled), color='orange', alpha=0.4, label="Sklearn Predictions")
plt.title(f"Part 2 (Sklearn): Target vs. Most Crucial Feature: {best_feat_name}")
plt.xlabel(best_feat_name)
plt.ylabel(TARGET_COL)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("part2_sklearn_feature_plot.png", dpi=150)
plt.close()

# 8. OUTPUT REPORT METRICS
final_pred = best_model.predict(X_test_scaled)
print("\n=== PART 2 SCYKIT-LEARN EVALUATION STATISTICS ===")
print(f"Optimal Model Config Found: LR={best_model.eta0}, Max_Iter={best_model.max_iter}")
...
for col, coef in zip(X_all.columns, best_model.coef_):
    print(f" Feature '{col}': {coef:.6f}")
print(f" Intercept Bias (b): {best_model.intercept_[0]:.6f}")